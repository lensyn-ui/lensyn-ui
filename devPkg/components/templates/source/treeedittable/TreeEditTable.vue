            ...
            <div v-for="item in treeArray" :style="[rowCls, item.edit ? editRowCls : {}]" :key="item.id" :row-id="item.id">
                <div v-for="(o, i) in item.d" :style="[columnCls, {'width': o.w + '%', 'padding-left': i === 0 ? (40+item.layer*20)+'px' : '20px'}]">
                    <div :style="columnBoxCls">
                        ...
                        <template v-if="o.t !== 'btn' && o.t !== 'icon'">
                            <span v-if="item.edit && o.f !== 'opt' && !o.d"
                                 :style="[o.f !== 'opt' && !o.d ? editTxtCls : '']"
                                 :node-type="o.f === 'opt' ? 'opt' : 'txt'"
                                 :now-idx="i"
                                 :now-name="o.f"
                                 :node-val="o.f !== 'opt' ? o.v : ''">
                                <input :value="o.v" :style="editInputCls" />
                            </span>

                            <span v-else :style="[txtCls, i === 0 ? treeTxtCls : '']"
                                :now-idx="i"
                                :now-name="o.f"
                                :node-type="o.f === 'opt' ? 'opt' : 'txt'"
                                :node-val="o.f !== 'opt' ? o.v : ''">
    ...
    export default{
        ...
        data() {
            return {
                ...
                editInputCls: {'width': '100%', 'height': '100%', 'outline': 'none', 'borderWidth': '0'},
        ...
        methods: {
            ...
            canShowOperate(rowData, row, operate) {
                ...
                if (!this.isCustomShowOperate || 
                    ((result = this.isCustomShowOperate(rowData, row, operate)) === null)) {
                ...
                obj.map(it => {
                    if(it.id+'' === id) {
                        ...
                        for(let k = 0;k < nodes.length;k ++) {
                            let nodeItem = nodes[k];
                            let key = nodeItem.getAttribute('now-name');
                            let idx = nodeItem.getAttribute('now-idx');
                            let editVal = "";
                            /**
                             * 如果是图标和弹出层类型的数据，需要取 node-val 里面的值，
                             * 如果是其中有输入框则取输入框中的值，其它的直接取 innerText 的值
                             */
                            if (it.d[idx].t === 'btn' || it.d[idx].t === 'icon') {
                                editVal = nodeItem.getAttribute("node-val");
                            } else {
                                if (nodeItem.children.length > 0 && nodeItem.children[0].tagName.toLowerCase() === "input") {
                                    editVal = nodeItem.children[0].value;
                                } else {
                                    editVal = nodeItem.innerText;
                                }
                            }
                            it[key] = editVal;
                            it.d[idx].v = editVal;
